...
const calcultePoints = async (id, matchHome, matchAway) => {
  User.find().then((users) => {
    users.forEach((user) => {
      const { results } = user;
      Bets.findOne({ match: id, user: user._id }).then((bet) => {
        if (bet) {
          const homeBet = bet.bet.home;
          const awayBet = bet.bet.away;
          ...
          if (
            (matchHome + matchAway >= 5 && homeBet + awayBet >= 5)
              && (matchHome === homeBet && matchAway === awayBet)
          ) {
            results.goals5 += 1;
          }
          ...
          User.updateOne({ _id: user._id }, { $set: { results } })
